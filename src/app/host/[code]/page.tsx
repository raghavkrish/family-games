...
export default function HostPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const stage = useMemo(() => {
    ...
    if (state.phase === "lobby" || state.phase === "pickGame") {
      return (
        <div className="grid h-full gap-6 lg:grid-cols-[1.1fr_1fr]">
          <div className="flex flex-col items-center justify-center gap-4 rounded-3xl border-4 border-ink bg-cream p-6 text-ink shadow-[8px_8px_0_#ff3d6e]">
            ...
            {onLocalhost && (
              <p className="rounded-xl bg-coral/20 px-3 py-2 text-left text-xs text-ink">
                ...
                <strong>http://&lt;your-wifi-ip&gt;:3000</strong> (see Next.js “Network” URL), then
                scan that QR so buzzers hit the same Wi‑Fi host.
              </p>
            )}
            <div className="mt-2 flex w-full gap-3">
              <div
                className={`flex-1 rounded-2xl border-4 border-ink px-3 py-3 text-center font-display ${
          ...
          <div className="flex flex-col gap-4">
            <div>
              <h2 className="font-display text-4xl text-cream">Pick a game</h2>
              <p className="text-cream/60">
                Wait for both buzzers, then smash a title.
              </p>
            </div>
            ...
            <div className="grid gap-3">
              {GAMES.map((g) => (
                <button
                  key={g.id}
                  type="button"
                  className={`rounded-3xl border-4 border-ink p-4 text-left shadow-[6px_6px_0_#111] ${g.accent}`}
